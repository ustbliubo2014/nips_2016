...
@author: liubo
@software: PyCharm
@file: stochastic_depth.py
@time: 2016/12/19 15:32
@contact: ustb_liubo@qq.com
@annotation: stochastic_depth
"""
import numpy as np
np.random.seed(2 ** 10)
...
X_train = X_train.astype('float32')[:1280]
X_test = X_test.astype('float32')[:1280]


# convert class vectors to binary class matrices
Y_train = np_utils.to_categorical(y_train, nb_classes)[:1280]
Y_test = np_utils.to_categorical(y_test, nb_classes)[:1280]
...
checkpointer = ModelCheckpoint(filepath="model/stochastic_depth.weights.{epoch:02d}-{val_loss:.2f}.hdf5", verbose=1)

model.save_weights('test.weight')
open('test.model').write(model.to_json())
...
model.fit_generator(datagen.flow(X_train, Y_train, batch_size=batch_size, shuffle=True),
                    ...
                    callbacks=[GatesUpdate(), LearningRateScheduler(schedule), checkpointer])
